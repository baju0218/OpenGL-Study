...
std::unique_ptr<Image> Image::Create(const std::string &filepath) {
  auto image = std::unique_ptr<Image>(new Image());
...
const uint8_t *Image::GetData() const { return m_data; }

int Image::GetWidth() const { return m_width; }

int Image::GetHeight() const { return m_height; }

int Image::GetChannelCount() const { return m_channelCount; }
...
bool Image::Allocate(int width, int height, int channelCount) {
  ...
  m_data = new uint8_t[m_width * m_height * m_channelCount];
